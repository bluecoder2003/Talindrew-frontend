  ...
  const hasDescription = React.Children.toArray(children).some((child) => {
    if (React.isValidElement(child)) {
      ...
      return child.type === DialogDescription ||
             (typeof child.type === 'function' && child.type.name === 'DialogDescription');
    }
    return false;
  });

  // If no description is found and no aria-describedby is provided, add a default one
  const contentProps = hasDescription || props['aria-describedby'] 
    ? props 
    : { ...props, 'aria-describedby': 'dialog-description-fallback' };
    ...
    <DialogPortal data-slot="dialog-portal">
      ...
      >
        ...
        {!hasDescription && !props['aria-describedby'] && (
          <span id="dialog-description-fallback" className="sr-only">
            Dialog content
          </span>
        )}
