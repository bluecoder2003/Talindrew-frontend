...
import { InputRefFunc, Annotation, Coor, CutSite, Highlight, Range, Size } from "./types";
import { CentralIndexContext } from "./contexts";
import { stackElements, isEqual, CHAR_WIDTH } from "./utils";
import { viewerCircular } from "./styles";
...
export default class Circular extends React.Component<CircularProps, CircularState> {
  ...
  static getDerivedStateFromProps = (nextProps: CircularProps): CircularState => {
    ...
    const zoomFactor = (nextProps.zoomLevel || 12) / 12; // Default zoom level is 12
    const lineHeight = baseLineHeight * zoomFactor;
    ...
    annotationsInRows.forEach((r: Annotation[]) => {
      ...
      r.forEach(ann => {
        ...
        const scaledCharWidth = CHAR_WIDTH * zoomFactor;
        const annNameLengthPixels = (ann.name.length + 2) * scaledCharWidth;
